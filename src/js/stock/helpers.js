...
const cargarDropDowns = () => {
  return Promise.all([
    $.ajax({ url: marcasURL, method: 'GET' }),
    $.ajax({ url: categoriasURL, method: 'GET' }),
    $.ajax({ url: proveedoresURL, method: 'GET' }),
    $.ajax({ url: unidadesMedidaURL, method: 'GET' })
  ]).then(([marcasData, categoriasData, proveedoresData, unidadesData]) => {
    ...
    $('#unidadMedida').html(unidadesOptions) // Llenar el select de unidades de medida
  ...
  $('#formFile').on('change', function (event) {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        ...
        const previewImage = $('#previewImage') // Seleccionar la imagen
